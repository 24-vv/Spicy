...
def settings():
    ...
    while True:
        ...
        print(f"{theme_color} [ 4 ] Back to Main Menu\033[0m")
        print()
        choice = input(f"{theme_color}[ SPICY ]\033[0m > \033[97m").strip()
        print()
        if choice == "1":
            auto_store = True
            print(f"{theme_color}[ Success ]\033[0m Auto Store enabled")
            print()
            input(f"{theme_color}[ SPICY ]\033[0m Press Enter to continue...")
        elif choice == "2":
            auto_store = False
            print(f"{theme_color}[ Success ]\033[0m Auto Store disabled")
            print()
            input(f"{theme_color}[ SPICY ]\033[0m Press Enter to continue...")
        elif choice == "3":
            ...
            print(f"{theme_color}[ ERROR ]\033[0m Unknown command. Type '1', '2', '3', or '4'.")
            print()
            input(f"{theme_color}[ SPICY ]\033[0m Press Enter to continue...")
    ...
    while True:
        ...
        if command[0] == "1":
            ...
            if auto_store:
                ...
                print(f'{theme_color}[ Success ]\033[0m Successfully Generated {len(proxies)} proxies')
            else:
                for proxy in proxies:
                    print(f'{theme_color}[ Success ]\033[0m Successfully Generated {proxy}')
            print()
        elif command[0] == "2":
            if auto_store:
            ...
            print()
            print(f'{theme_color}[ SPICY ]\033[0m Goodbye!')
            break
        else:
            print()
            print(f"{theme_color}[ ERROR ]\033[0m Unknown command. Type '1', '2', '3', or '4'.")
